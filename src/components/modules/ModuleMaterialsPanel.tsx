...
import { Box, Stack, Typography, Button, Paper, IconButton, Tooltip, CircularProgress } from '@mui/material'
...
export default function ModuleMaterialsPanel({ moduloId }: Props) {
  ...
  const iconFor = (tipo: string) => {
    if (tipo.includes('pdf')) return <PictureAsPdf fontSize='small' color='error' />
  ...
  return (
    <Paper variant='outlined' sx={{ p: 2, display: 'grid', gap: 2 }}>
      <Stack direction='row' justifyContent='space-between' alignItems='center'>
        ...
        <Button component='label' startIcon={<CloudUploadIcon />} size='small' disabled={uploading}>
      ...
      {isLoading ? (
        ...
        <Typography variant='body2' color='text.secondary'>Nenhum material.</Typography>
      ) : (
        <Stack gap={1}>
          {materials.map(m => (
            <Paper key={m.id} variant='outlined' sx={{ p: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
              {iconFor(m.tipo_arquivo)}
              <Box sx={{ flex: 1 }}>
                <Typography variant='body2' fontWeight={600}>{m.nome_arquivo}</Typography>
                <Typography variant='caption' color='text.secondary'>{(m.tamanho / 1024).toFixed(1)} KB</Typography>
              </Box>
