...
import { Box, TextField, Grid, FormControlLabel, Switch, Button, Stack, FormControl, InputLabel, Select, MenuItem } from '@mui/material'
...
export default function ModuleInfoForm({ cursoCodigo, modulo, onSaved }: Props) {
  ...
  const [tipoConteudo, setTipoConteudo] = useState<string>(modulo.tipo_conteudo || 'texto')
  ...
  const handleSave = async () => {
    await updateModule.mutateAsync({ titulo, ordem, xp, obrigatorio, conteudo, tipo_conteudo: tipoConteudo })
    onSaved?.()
  }
  ...
  return (
    <Box sx={{ display: 'grid', gap: 2 }}>
      ...
      <Stack direction='row' justifyContent='flex-end'>
        <Button variant='contained' size='small' onClick={handleSave} disabled={updateModule.isPending || !titulo.trim()}>Salvar</Button>
      </Stack>
    </Box>
  )
}
